...
float calculateDistanceRemaining() {
  float distanceRemaining = calculateDistance(robotX, robotY, waypoints[currentWaypoint][0], waypoints[currentWaypoint][1]);
  for (int i = currentWaypoint; i < numWaypoints - 1; i++) {
    // Serial.println(distanceRemaining);
    distanceRemaining += calculateDistance(waypoints[i][0], waypoints[i][1], waypoints[i+1][0], waypoints[i+1][1]);
  }
  // Serial.println(distanceRemaining);
...
void purePursuit() {
  ...
  // if (currentWaypoint == numWaypoints - 1 && millis() - startTime < desiredTotalDuration) {
  if (millis() - startTime < desiredTotalDuration) {
    ...
    double distanceTraveled = totalDistanceOfPath - distanceRemaining;
    ...
    Serial.print("Input:"); Serial.print(inputPurePursuit, 5);
    Serial.print(",SetPoint:"); Serial.print(setPointPurePursuit, 5);
    Serial.print(",Multiplier:"); Serial.print(multiplier, 5);
    Serial.print(",TotalDistanceOfPath:"); Serial.print(totalDistanceOfPath, 5);
    Serial.print(",DistanceRemaining:"); Serial.print(distanceRemaining, 5);
    Serial.print(",DistanceTraveled:"); Serial.print(distanceTraveled, 5);
